...
// maplibre-gl v6 ships ESM with no default export; the mapcn registry template
// assumes one. Namespace import keeps `MapLibreGL.Map/Marker/Popup` intact.
...
// maplibre derives its worker URL from `import.meta.url` and returns "" unless
// that URL is http(s). Turbopack's bundled module URL is not, so it falls back
// to `new Worker("")` — which loads the HTML document as a module script. The
// worker dies instantly and silently: no error event fires, but every source
// stays unloaded forever (`map.loaded()` never becomes true, zero features).
// scripts/copy-maplibre-worker.mjs serves the worker (and the shared chunk it
// imports relatively) from /maplibre/, so point maplibre at it explicitly.
